...
- **Multi-Tenancy**: Every backend route and DB query is scoped by `store_domain` for secure multi-tenant operation. Postgres Row-Level Security (RLS) policies with FORCE ROW LEVEL SECURITY are enabled on all tenant-scoped tables (`conversations`, `sessions`, `analytics_logs`, `shop_knowledge`, `user_preferences`, `mcp_connections`). Default behavior is fail-closed (no rows visible without tenant context). All tenant-scoped queries use `withTenantScope(storeDomain, fn)` which sets `app.current_store_domain` via `SET LOCAL` inside a transaction. Administrative/maintenance operations use `withAdminBypass(fn)` which sets `app.rls_bypass='on'` inside a transaction. Cross-tenant guard middleware detects and blocks session/conversation ownership violations. Audit logs table tracks all sensitive operations (store CRUD, auth events, knowledge mutations, MCP operations, tool executions).
- **Audit Logging**: `audit_logs` table records all security-sensitive operations with actor, action, resource type/id, metadata, IP address, and timestamp. The `logAuditFromRequest()` helper extracts context from Express requests. Cross-tenant access attempts are logged with full attribution via `logCrossTenantAttempt()`.
...
- **Audit Logging**: Dedicated `audit_logs` table records all sensitive mutations (store CRUD, merchant auth, token changes, knowledge mutations, MCP operations, cross-tenant access attempts) with actor, action, resource, metadata, IP address, and timestamp. The `audit-logger` service provides fire-and-forget logging with `logAuditFromRequest()` for route handlers and `logCrossTenantAttempt()` for security events.
